...
class ClusteringLayer(Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        q = 1.0 / (1.0 + ((K.min(K.sqrt(K.sum(K.square(K.expand_dims((2*self.clusters)-K.expand_dims(inputs, axis=1),axis=1)-K.expand_dims(inputs,axis=1)),axis=-1)),axis=1)) / self.alpha))
...
class DEC(object):
    ...
    def fit(self, x, y=None, maxiter=2e4, batch_size=256, tol=1e-3,
        ...
        print(np.asarray(self.encoder.predict(x)).shape)
